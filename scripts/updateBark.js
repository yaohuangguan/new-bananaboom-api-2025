...
// 引入你的 User 模型 (确保路径对)
import { findOne, updateOne, findById } from '../models/User';
...
async function main() {
  try {
    ...
    // 查找用户是否存在
    const user = await findOne({ email: email.trim() });
    if (!user) {
    ...
    const result = await updateOne({ _id: user._id }, { $set: { barkUrl: cleanUrl } });
    ...
    // 4. (可选) 验证一下，显式查出来看看
    const verifyUser = await findById(user._id).select('+barkUrl');
