...
import { getUserMergedPermissions } from '../services/permissionService.js'; // 🔥 改动1: 引入权限服务

export default function (requiredPerm) {
  return (req, res, next) => {
    // 1. 基础鉴权：确保用户已登录
    if (!req.user) {
    ...
    const rolePerms = getUserMergedPermissions(user);
